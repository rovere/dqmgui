...
void VisDQMRenderLink::initimg(Image &proto,
                               const std::string &path,
...
void  VisDQMRenderLink::lrunuke(Image &img) {
  img.id = 0;
  img.hash = 0;
  img.version = 0;
  img.flags = 0;
...
VisDQMRenderLink::Image &
VisDQMRenderLink::lruimg(int &imgidx, Image &proto) {
  int lruidx = -1;
  imgidx = -1;
...
void VisDQMRenderLink::requestimg(Image &img, std::string &imgbytes,
...
void VisDQMRenderLink::resizeimg(std::string &imgdata,
                                 Image &destimg, const Image &srcspec) {
...
void VisDQMRenderLink::compress(Image &img, const std::string &imgbytes) {
  assert(!imgbytes.empty());
...
void VisDQMRenderLink::expandpng(std::string &imgbytes, const Image &img) {
  // FIXME: Lock, unlock?
  FILE *png = 0;
  try {
