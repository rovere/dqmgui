...
void
VisDQMArchiveSource::loadIndex(void)
{
...
bool
VisDQMArchiveSource::fileReadFailure(int ntries, const char *message)
{
...
void
VisDQMArchiveSource::maybeReloadIndex(void)
{
...
VisDQMArchiveSource::open(uint16_t kind, const uint32_t *file)
{
  ...
  if (pos == files.end())
  {
    VisDQMArchiveSource::VisDQMFilePtr ptr(index_.open(kind,
                                                       file[kind] >> 16,
                                                       file[kind] & 0xffff,
...
void
VisDQMArchiveSource::getdata(const VisDQMSample &sample,
  ...
  for (int ntries = 1; true; ++ntries)
  {
    try
    {
      ...
      if (si != samples_.end() && objindex != 0)
      {
        ...
        {
          VisDQMArchiveSource::VisDQMFilePtr file(open(VisDQMIndex::MASTER_FILE_INFO, si->files));
        ...
        {
          VisDQMArchiveSource::VisDQMFilePtr file(open(VisDQMIndex::MASTER_FILE_DATA, si->files));
          VisDQMFile::ReadHead rdhead(file.get(), keyidx);
          if (! rdhead.isdone())
          {
            rdhead.get(&key, &begin, &end);
...
void
VisDQMArchiveSource::getattr(const VisDQMSample &sample,
                             const std::string &path,
...
py::str
VisDQMArchiveSource::getJson(const int runnr,
...
py::tuple
VisDQMArchiveSource::plot(int runnr,
                          const std::string &dataset,
                          const std::string &path,
                          py::dict opts)
{
...
// seconds.
void
VisDQMArchiveSource::update(VisDQMItems &items,
...
void
VisDQMArchiveSource::prepareSession(py::dict session)
{
...
void
VisDQMArchiveSource::scan(VisDQMItems &result,
                          const VisDQMSample &sample,
                          VisDQMEventNum &current,
                          VisDQMRegexp *rxmatch,
                          Regexp *rxsearch,
...
void
VisDQMArchiveSource::json(const VisDQMSample &sample,
                          ...
                          std::set<std::string> &dirs)
{
  ...
  for (int ntries = 1; true; ++ntries)
  {
    try
    {
      ...
      // loop around to retry a certain number of times.  The
      // stack unwinding automatically handles lock release.
      VisDQMArchiveSource::VisDQMFilePtr info(open(VisDQMIndex::MASTER_FILE_INFO, si->files));
      VisDQMArchiveSource::VisDQMFilePtr data(open(VisDQMIndex::MASTER_FILE_DATA, si->files));
...
void
VisDQMArchiveSource::getcert(VisDQMSample &sample,
                             const std::string &rootpath,
...
void
VisDQMArchiveSource::samples(VisDQMSamples &samples)
{
  try
  {
    // Reload index if necessary.
