...
static std::string sampleToJSON(const VisDQMSample &sample) {
  std::string result;
...
static std::string samplesToJSON(VisDQMSamples &samples) {
  std::string result;
...
static VisDQMSample sessionSample(const py::dict &session) {
  VisDQMSample result;
  ...
  result.importversion =
      py::extract<int>(session.get("dqm.sample.importversion"));
  result.origin = 0;

  if (type < SAMPLE_LIVE || type > SAMPLE_OFFLINE_MC)
    throw std::runtime_error(
        StringFormat("invalid session dqm.sample.type: %1").arg(type));

  result.type = (VisDQMSampleType)type;

  if (result.runnr < 0)
    throw std::runtime_error(
        StringFormat("invalid session dqm.sample.runnr: %1").arg(result.runnr));
  return result;
}
...
class VisDQMSource {
 protected:
  ...
  void clearattr(VisDQMIndex::Summary &attrs) {
 ...
 public:
  ...
  virtual const char *plotter(void) const { return "unknown"; }

  virtual const char *jsoner(void) const { return "unknown"; }

  virtual void prepareSession(py::dict /* session */) {}

  virtual void getdata(const VisDQMSample & /* sample */,
                       const std::string & /* path */, std::string &streamers,
                       DQMNet::Object &obj) {
    streamers.clear();
    clearobj(obj);
  }
  ...
  virtual void scan(VisDQMItems & /* result */,
                    ...
                    bool * /* alarm */, std::string * /* layoutroot */,
                    VisDQMRegexp * /* rxlayout */) {}

  virtual void json(const VisDQMSample & /* sample */,
                    const std::string & /* rootpath */, bool /* fulldata */,
                    bool /* lumisect */, std::string & /* result */,
...
static IMGOPT TRENDIMGOPTS[] = {{"w", RX_OPT_INT},
                                {"h", RX_OPT_INT},
...
class VisDQMRenderLink {
 ...
 public:
  ...
  VisDQMRenderLink(const std::string &basedir, const std::string &logdir,
                   const std::string &plugin, int nproc, bool debug) {
    assert(!s_instance);
    s_instance = this;
    plugin_ = plugin;
    debug_ = debug;
  ...
  bool render(std::string &image, std::string &type, const std::string &path,
              const std::map<std::string, std::string> &opts,
              const std::string *streamers, DQMNet::Object *obj, size_t numobj,
              IMGOPT *IMGOPTS) {
    ...
    if (destwidth <= 0 || destheight <= 0 || destwidth >= 2500 ||
        destheight >= 2500)
      return false;
    ...
    char buf[64];
    sprintf(buf, "%sw=%d;h=%d", reqspec.empty() ? "" : ";", destwidth,
            destheight);
    ...
    Lock gate(&lock_);
    return !quiet_ &&
           makepng(image, type, protoimg, finalspec, destwidth, destheight);
  }

  bool prepareJson(std::string &jsonData, std::string &type,
                   const std::string &path,
                   const std::map<std::string, std::string> &opts,
                   const std::string *streamers, DQMNet::Object *obj,
                   size_t numobj, IMGOPT *) {
    std::string reqspec;
    Image protoimg;
 ...
 private:
  ...
  void startsrv(Server &srv) {
    ...
    IOChannel *null = 0;
    const char *serverArgz[] = {"visDQMRender",
                                "--state-directory",
                                srv.path.name(),
                                "--load",
                                plugin_.c_str(),
  ...
  static void initimg(Image &proto, const std::string &path,
                      const std::string &imagespec,
                      const std::string *streamers, const DQMNet::Object *obj,
                      size_t numobj, int width = 0, int height = 0) {
    assert(numobj);
    lrunuke(proto);
    ...
    for (size_t i = 0; i < numobj; ++i) {
      ...
      if (isROOTType(obj[i].flags) || isBLOBType(obj[i].flags))
        datalen +=
            2 * sizeof(uint32_t) + streamers[i].size() + obj[i].rawdata.size();
      else
  ...
  void requestimg(Image &img, std::string &imgbytes, const bool json = false,
    ...
    for (size_t i = 0; i < servers_.size(); ++i) {
      ...
      if (srv.checkme && p && p->done()) {
        ...
        logme() << "WARNING: restarting render process #" << i << ", process "
                << pid << buf << " while rendering '" << srv.lastimg << "'\n";
        blacklistimg(srv);
        srv.socket.abort();
        srv.proc.reset();
        srv.pending.clear();
  ...
  void resizeimg(std::string &imgdata, Image &destimg, const Image &srcspec) {
    ...
    try {
      ...
      if (imgdata.size() ==
          size_t(srcspec.width) * size_t(srcspec.height) * 3) {
        rgb8c_view_t srcview =
            interleaved_view(srcspec.width, srcspec.height,
                             (rgb8c_pixel_t *)&imgdata[0], srcspec.width * 3);
      ...
      logme() << "WARNING: failed to scale image: " << e.what() << std::endl;
  ...
  void compress(Image &img, const std::string &imgbytes) {
    ...
    try {
      ...
      logme() << "WARNING: failed to encode png image: " << e.what()
              << std::endl;
  ...
  bool makepng(std::string &imgdata, std::string &imgtype, Image &protoreq,
               const std::string &spec, int width, int height) {
    imgdata.clear();
    imgtype.clear();
    int imgidx;
  ...
  bool makeJson(std::string &jsonData, std::string &imgtype, Image &protoreq,
                const std::string &spec, const bool jsroot) {
    jsonData.clear();
    imgtype.clear();
    Image proto = protoreq;
    proto.id = Time::current().ns();
  ...
  void expandpng(std::string &imgbytes, const Image &img) {
    ...
    try {
      ...
      logme() << "WARNING: failed to decode png image: " << e.what()
              << std::endl;
      imgbytes.clear();
    } catch (...) {
 ...
 public:
  ...
  py::tuple plot(const std::string &path, py::dict opts) {
...

    {
      ...
      streamers.clear();
      clearobj(obj);
      imageok = link_->render(imagedata, imagetype, path, options, &streamers,
                              &obj, 1, STDIMGOPTS);
    }
...
class VisDQMToJSON {
 public:
  ...
  py::tuple list(VisDQMSource *layoutsrc, VisDQMSource *src, int runnr,
...

    {
      ...
      src->json(sample, path, options.count("rootcontent") > 0,
                options.count("lumisect") > 0, result, stamp, "", dirs, rxname);

      // Add layout content, in case a layout source had been
      // registered to the main server.
      if (layoutsrc) {
        layoutsrc->scan(layoutResult, sample, current, &rxmatch, rxsearch,
                        &alarm, &layoutroot, &rxlayout);
  ...
  void json(const VisDQMItems &contents, VisDQMSample &sample,
            VisDQMSource *src,
            const std::map<std::string, std::string> &options, double &stamp,
            const std::string &rootpath, std::string &result,
...
class VisDQMOverlaySource : public VisDQMSource {
 ...
 public:
  ...
  virtual const char *jsoner(void) const { return "underconstruction"; }
  ...
  py::tuple plot(py::list overlay, py::dict opts) {
    ...
    if (objects.size()) {
      PyReleaseInterpreterLock nogil;
      imageok =
          link_->render(imagedata, imagetype, path, options, &streamers[0],
                        &objects[0], objects.size(), STDIMGOPTS);
    } else {
      PyReleaseInterpreterLock nogil;
      streamer.clear();
      clearobj(obj);
      imageok = link_->render(imagedata, imagetype, path, options, &streamer,
                              &obj, 1, STDIMGOPTS);
    }
...
class VisDQMStripChartSource : public VisDQMSource {
 ...
 public:
  ...
  py::tuple plot(py::list pysources, py::object info, const std::string &path,
                 py::dict opts) {
    ...
    // Now let python go and start doing real work.
    {
      ...
      for (size_t i = 0, e = final.size(); i != e; ++i) {
        ...
        if (skipempty && !oneattr.nentries && final[i].type != SAMPLE_LIVE)
          continue;
      ...
      if (attrs.size()) {
        ...
        imageok =
            link_->render(imagedata, imagetype, path, options, streamers, xobj,
                          (xobj[1].rawdata.size() ? 2 : 1), TRENDIMGOPTS);
      } else {
        imageok = link_->render(imagedata, imagetype, path, options, streamers,
                                xobj, 1, STDIMGOPTS);
      }
  ...
  py::tuple plot(py::object info, const std::string &fullpath,
    ...
    if (info.ptr() != Py_None) {
      ...
      if (src.check()) {
        ...
        src()->getcert(cursample, path, variableName, attrs, axisvals,
                       binlabels);
        clearobj(xobj[0]);
        clearobj(xobj[1]);
        if (attrs.size()) {
          ...
          memcpy(&xobj[0].rawdata[0], &axisvals[0], axissize);
          memcpy(&xobj[0].rawdata[axissize], &attrs[0], attrsize);
          memcpy(&xobj[0].rawdata[axissize + attrsize], &binlabels[0],
                 binssize);
          imageok = link_->render(imagedata, imagetype, fullpath, options,
                                  streamers, xobj, 1, TRENDIMGOPTS);
        } else {
...
class VisDQMLayoutSource : public VisDQMSource {
 ...
 public:
  ...
  virtual void scan(VisDQMItems &result, const VisDQMSample & /* sample */,
                    ...
                    std::string *layoutroot, VisDQMRegexp *rxlayout) {
    Lock gate(&lock_);
        ...
        for (; ri != re; ++ri) {
          ...
          for (; ci != ce; ++ci) {
            ...
            if (pycol.ptr() == Py_None)
              ...
              col->path = StringAtom(
                  &stree, py::extract<std::string>(colobj.get("path")));
              col->desc =
                  py::extract<std::string>(colobj.get("description", ""));
              if (colobj.has_key("draw"))
                translateDrawOptions(py::extract<py::dict>(colobj.get("draw")),
                                     col->drawopts);
              if (colobj.has_key("overlays")) {
                py::list overlays =
                    py::extract<py::list>(colobj.get("overlays"));
              ...
              logwarn() << "layout element " << i->name.string() << "["
...
class VisDQMLiveThread : public DQMBasicNet {
 ...
 public:
  ...
  virtual bool onMessage(Bucket *msg, Peer *p, unsigned char *data,
                         size_t len) {
  ...
  void fetch(const std::string &name, std::string &streamers, Object &retobj) {
    ...
    uint32_t header[3] = {(uint32_t)req.size(), DQM_MSG_GET_OBJECT,
                          (uint32_t)name.size()};
  ...
  void scan(VisDQMItems &result, VisDQMEventNum &current, VisDQMRegexp *rxmatch,
            Regexp *rxsearch, bool *alarm) {
    ...
    for (di = items_.begin(), de = items_.end(); di != de; ++di) {
      ...
      if (isIntegerType(i.flags) &&
          i.name.string().find("/EventInfo/i") != std::string::npos)
        getEventInfoNum(i.name.string(), i.data.c_str(), eventnums);
      else if (isRealType(i.flags) &&
               i.name.string().find("/EventInfo/ru") != std::string::npos)
        getEventInfoNum(i.name.string(), i.data.c_str(), eventnums);

      if (!fastmatch(rxmatch, i.name) ||
          (rxsearch && rxsearch->search(i.name.string()) < 0) ||
          (alarm && ((i.flags & VisDQMIndex::SUMMARY_PROP_REPORT_ALARM) != 0) !=
                        (*alarm == true)))
        continue;
  ...
  void json(const std::string &rootpath, bool fulldata, bool /* lumisect */,
            std::string &result, double &stamp, const std::string &mename,
            std::set<std::string> &dirs, shared_ptr<Regexp> rxname) {
 ...
 protected:
  ...
  virtual void removePeer(Peer *p, Socket *s) {
...
class VisDQMLiveSource : public VisDQMSource {
 ...
 public:
  ...
  virtual void getattr(const VisDQMSample & /* sample */,
                       const std::string &path, VisDQMIndex::Summary &attrs,
                       std::string &xstreamers, DQMNet::Object &xobj) {
    clearattr(attrs);
...
class VisDQMArchiveWatch {
 ...
 public:
  ...
  bool running(void) { return thread_ != (pthread_t)-1; }
...
class VisDQMArchiveSource : public VisDQMSource {
  ...
  bool fileReadFailure(int ntries, const char *message) {
    ...
    loginfo() << (ntries >= 0 ? ntries : 1) << " transient error"
              << (ntries > 1 ? "s" : "") << " while reading data, "
              << (abandon ? "giving up" : "retrying")
  ...
  VisDQMFilePtr open(uint16_t kind, const uint32_t *file) {
    FileMap &files =
        (kind == VisDQMIndex::MASTER_FILE_INFO ? infoFiles_ : dataFiles_);
    ...
    if (pos == files.end()) {
      VisDQMFilePtr ptr(index_.open(kind, file[kind] >> 16, file[kind] & 0xffff,
  ...
  SampleList::iterator findSample(const VisDQMSample &sample) {
    ...
    for (i = samples_.begin(), e = samples_.end(); i != e; ++i)
      if (i->numObjects > 0 && i->runNumber == sample.runnr &&
 ...
 public:
  ...
  virtual void getdata(const VisDQMSample &sample, const std::string &path,
    ...
    for (int ntries = 1; true; ++ntries) {
      try {
        ...
        uint32_t objindex =
            StringAtom(&objnames_, path, StringAtom::TestOnly).index();
  ...
  virtual void getattr(const VisDQMSample &sample, const std::string &path,
                       VisDQMIndex::Summary &attrs,
                       std::string & /* xstreamers */,
                       DQMNet::Object & /* xobj */) {
    clearattr(attrs);
  ...
  py::str getJson(const int runnr, const std::string &dataset,
  ...
  py::tuple plot(int runnr, const std::string &dataset, const std::string &path,
                 py::dict opts) {
    VisDQMSample sample(SAMPLE_ANY, runnr, dataset);
    std::map<std::string, std::string> options;
    bool imageok = false;
  ...
  // seconds.
  void update(VisDQMItems &items, const VisDQMSample &sample,
    ...
    for (int ntries = 1; true; ++ntries) {
      try {
        ...
        for (VisDQMFile::ReadHead rdhead(file.get(), keyidx); !rdhead.isdone();
             rdhead.next()) {
          IndexKey key;
          void *begin;
          void *end;
          ...
          if (isIntegerType(s->properties) &&
              name.find("/EventInfo/i") != std::string::npos)
            getEventInfoNum(name, data, eventnums);
          else if (isRealType(s->properties) &&
                   name.find("/EventInfo/ru") != std::string::npos)
            getEventInfoNum(name, data, eventnums);
  ...
  virtual void prepareSession(py::dict session) {
    if (!session.has_key("dqm.sample.type")) {
...

      {
        ...
        for (newest = si = samples_.begin(), se = samples_.end(); si != se;
             ++si)
  ...
  virtual void json(const VisDQMSample &sample, const std::string &rootpath,
    ...
    for (int ntries = 1; true; ++ntries) {
      try {
        ...
        if (result.find("streamerinfo") == std::string::npos)
          result +=
              StringFormat("%1{ \"streamerinfo\":\"%2\" }\n")
                  .arg(result.empty() ? "" : ", ")
                  .arg(fulldata ? hexlify(streamers_.key(si->streamerInfoIdx))
                                : std::string());
  ...
  virtual void getcert(VisDQMSample &sample, const std::string &rootpath,
                       const std::string &variableName,
                       std::vector<VisDQMIndex::Summary> &attrs,
                       std::vector<double> &axisvals, std::string &binlabels) {
    attrs.reserve(4000);
...
class VisDQMWorkspace {
 ...
 public:
  ...
  virtual std::string state(py::dict /* session */) { return "{}"; }

  virtual void profilesnap(void) {
    if (snapdump_) {
      char tofile[256];
      struct timeval tv;
      gettimeofday(&tv, 0);
      if (snprintf(tofile, sizeof(tofile), "|gzip -c>igprof.dqmgui.%ld.%f.gz",
                   (long)getpid(),
                   tv.tv_sec + 1e-6 * tv.tv_usec) < int(sizeof(tofile)))
        snapdump_(tofile);
    }
  }
 ...
 protected:
  ...
  template <class T>
  T workspaceParam(const py::dict &session, const char *key, const T &def) {
    py::dict d = py::extract<py::dict>(session.get(key));
    return py::extract<T>(d.get(this->workspaceName_, def));
  }

  template <class T>
  T workspaceParamOther(const py::dict &session, const char *key,
  ...
  template <class T>
  T workspaceParamOther(const py::dict &session, const char *key, const T &def,
                        const std::string &wspace) {
    py::dict d = py::extract<py::dict>(session.get(key));
  ...
  static std::string sessionPanelConfig(const py::dict &session,
                                        const char *panel) {
    char attrname[64];
    bool show;
    int x;
  ...
  std::string makeResponse(
    ...
    StringFormat result =
        StringFormat(
            ...
            .arg(strip)
            .arg(StringFormat(
                     "{'pattern':%1, 'valid':%2, 'nmatches':%3, 'error':%4}")
  ...
  const std::string &workspaceListJSON(void) {
    if (guiWorkspaceListJSON_.empty()) {
      ...
      for (py::stl_input_iterator<py::object> i(workspaces), e; i != e; ++i) {
        ...
        parts.push_back(
            StringFormat(
                "{'title':'%1', 'label':'%2', 'category':'%3', 'rank':%4}")
                .arg(name)
                .arg(lower)
                .arg(category)
                .arg(rank));
  ...
  void buildContents(const std::vector<VisDQMSource *> &sources,
                     ...
                     StringAtomSet &myobjs, const VisDQMSample &sample,
                     VisDQMEventNum &current,
    ...
    for (size_t i = 0, e = sources.size(); i != e; ++i) {
      sources[i]->scan(contents, sample, current, rxmatch, rxsearch, alarm,
                       layoutroot, rxlayout);
    }
    ...
    for (mi = myobjs.begin(), me = myobjs.end(); mi != me; ++mi) {
      std::string trailer(mi->string(),
                          StringOps::rfind(mi->string(), '/') + 1);
  ...
  void buildShown(const VisDQMItems &contents, StringAtomSet &shown,
  ...
  void updateStatus(const VisDQMItems &contents, VisDQMStatusMap &status) {
    VisDQMStatus empty;
    empty.nleaves = 0;
    empty.nalarm = 0;
    empty.nlive = 0;
    ...
    for (ci = contents.begin(), ce = contents.end(); ci != ce; ++ci) {
        ...
        for (size_t nrow = 0; nrow < rows.size(); ++nrow) {
          ...
          for (size_t ncol = 0; ncol < row.columns.size(); ++ncol) {
            ...
            if ((lpos = contents.find(col.path)) != contents.end())
              s.nalarm += ((lpos->second->flags &
                            VisDQMIndex::SUMMARY_PROP_REPORT_ALARM)
                               ? 1
                               : 0);
      ...
      for (size_t i = 0, e = o.parents.size(); i != e; ++i) {
        ...
        if (pos == status.end())
          pos = status.insert(VisDQMStatusMap::value_type(o.parents[i], empty))
                    .first;
...
class VisDQMSummaryWorkspace : public VisDQMWorkspace {
 ...
 public:
  ...
  virtual std::string state(py::dict session) {
    ...
    std::string showerrbars(
        py::extract<std::string>(session.get("dqm.showerrbars")));
    ...
    // Now do the hard stuff, out of python.
    {
      ...
      buildContents(srclist, contents, myobjs, sample, current, 0,
                    rxsearch.get(), 0, 0, 0);
      ...
      for (ci = contents.begin(), ce = contents.end(); ci != ce; ++ci) {
        ...
        if (qalarm) {
          StringAtom reportSummary(&stree, system + "/EventInfo/reportSummary",
                                   StringAtom::TestOnly);
        ...
        if (info.empty()) {
          plotter = StringFormat("%1/%2%3")
                        ...
                        .arg(sample.runnr)
                        .arg(sample.dataset);
          info["MEs"] = StringFormat("%1").arg(
              status[StringAtom(&stree, system)].nleaves);
      ...
      return makeResponse(StringFormat("{'kind':'DQMSummary', 'items':[%1],"
                          ...
                          services, workspaceName_, workspaces, submenu, sample,
                          filter, showstats, showerrbars, reference, strip,
                          rxstr, rxerr, summary.size(), toolspanel, startTime);
    }
  }
 ...
 public:
  ...
  virtual std::string state(py::dict session) {
    ...
    // Now do the hard stuff, out of python.
    {
      ...
      return makeResponse(
          StringFormat("{\"kind\":\"DQMCertification\", \"items\":[%1],"
                       " \"current\":\"%2\", %3}")
              .arg(certificationToJSON(cert))
              .arg(plotter)
              .arg(zoom),
          sample.type == SAMPLE_LIVE ? 30 : 300, current, services,
          workspaceName_, workspaces, submenu, sample, filter, showstats,
          showerrbars, reference, strip, rxstr, rxerr, cert.size(), toolspanel,
          startTime);
    }
  }
    ...
    // Now do the hard stuff, out of python.
    {
      ...
      for (ci = contents.begin(), ce = contents.end(); ci != ce; ++ci) {
        ...
        if (!isScalarType(o.flags)) {
          ...
          StringAtom eventTimeStamp(&stree, label + "/EventInfo/eventTimeStamp",
                                    StringAtom::TestOnly);
      ...
      return makeResponse(StringFormat("{'kind':'DQMQuality', 'items':[%1]}")
                          ...
                          services, workspaceName_, workspaces, submenu, sample,
                          filter, showstats, showerrbars, reference, strip,
                          rxstr, rxerr, qmap.size(), toolspanel, startTime);
    }
  }
 ...
 private:
  static std::string qmapToJSON(const QMap &qmap) {
    ...
    for (i = qmap.begin(), e = qmap.end(); i != e; ++i)
      len += 100 + 2 * (i->first.size() + i->second.location.size() +
                        i->second.name.size() + i->second.reportSummary.size() +
...
class VisDQMSampleWorkspace : public VisDQMWorkspace {
 public:
  ...
  virtual std::string state(py::dict session) {
    ...
    std::string order(
        py::extract<std::string>(session.get("dqm.sample.order")));
    std::string dynsearch(
        py::extract<std::string>(session.get("dqm.sample.dynsearch")));
    std::string pat(
        py::extract<std::string>(session.get("dqm.sample.pattern")));
...
class VisDQMLayoutTools {
 protected:
  ...
  static std::string layoutToJSON(const VisDQMShownItem &x,
                                  const VisDQMSample &sample) {
    char vbuf[64];
  ...
  static std::string layoutToJSON(const VisDQMShownRow &layout,
                                  const VisDQMSample &sample) {
  ...
  static std::string layoutToJSON(const VisDQMShownRows &layout,
                                  const VisDQMSample &sample) {
  ...
  static std::string shownDirToJSON(const VisDQMItems & /* contents */,
                                    const VisDQMStatusMap &status,
                                    const StringAtom &x) {
    assert(status.count(x));
    const VisDQMStatus &info = status.find(x)->second;
    return StringFormat(
               "{'name':%1, 'dir':{'nalarm':%2, 'nlive':%3, 'nleaves':%4}}")
  ...
  static std::string shownObjToJSON(const VisDQMItems &contents,
                                    const VisDQMStatusMap &status,
                                    const VisDQMDrawOptionMap &drawopts,
                                    const VisDQMSample &sample,
                                    const StringAtom &x) {
  ...
  static void setDrawOptions(VisDQMDrawOptions *to,
  ...
  static void layoutOne(VisDQMShownRows &layout, const VisDQMItems &contents,
                        const VisDQMStatusMap &status,
                        const VisDQMDrawOptionMap &drawopts,
                        const StringAtom &path) {
...
//////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
class VisDQMContentWorkspace : public VisDQMWorkspace,
                               public VisDQMLayoutTools {
 ...
 public:
  ...
  virtual std::string state(py::dict session) {
    ...
    std::string rxstr(py::extract<std::string>(session.get("dqm.search")));
    py::dict pydrawopts(
        workspaceParam<py::dict>(session, "dqm.drawopts", py::dict()));
    py::tuple myobjst(
        workspaceParam<py::tuple>(session, "dqm.myobjs", py::tuple()));
    ...
    // Now do the hard stuff, out of python.
    {
      ...
      buildContents(srclist, contents, myobjs, sample, current, &rxmatch_,
                    rxsearch.get(), qalarm, &layoutroot, &rxlayout_);
      ...
      return makeResponse(
          StringFormat("{'kind':'DQMCanvas', 'items':%1,"
              ...
              .arg(shownToJSON(contents, status, drawopts,
                               StringAtom(&stree, root), sample, shown))
          ...
          sample.type == SAMPLE_LIVE ? 30 : 300, current, services,
          workspaceName_, workspaces, submenu, sample, filter, showstats,
          showerrbars, reference, strip, rxstr, rxerr, contents.size(),
 ...
 public:
  ...
  virtual std::string state(py::dict session) {
    ...
    std::string workspace(
        py::extract<std::string>(session.get("dqm.play.prevws")));
    std::string root(
        workspaceParamOther<std::string>(session, "dqm.root", "", workspace));
    std::string filter(py::extract<std::string>(session.get("dqm.filter")));
    std::string showstats(
        py::extract<std::string>(session.get("dqm.showstats")));
    std::string showerrbars(
    ...
    py::dict pydrawopts(workspaceParamOther<py::dict>(session, "dqm.drawopts",
                                                      py::dict(), workspace));
    py::tuple myobjst(workspaceParamOther<py::tuple>(session, "dqm.myobjs",
                                                     py::tuple(), workspace));
    StringAtomSet myobjs;
    ...
    // Now do the hard stuff, out of python.
    {
      ...
      return makeResponse(
          StringFormat("{'kind':'DQMPlay', 'items':%1,"
              ...
              .arg(showstats)
              .arg(showerrbars),
          300, current, services, workspace, workspaces, submenu, sample,
          filter, showstats, showerrbars, reference, strip, rxstr, rxerr,
          contents.size(), toolspanel, startTime);
    }
  }
};
...
BOOST_PYTHON_MODULE(Accelerator) {
  ...
  Signal::handleFatal(
      0, IOFD_INVALID, 0, 0,
      (Signal::FATAL_DEFAULT & ~(Signal::FATAL_ON_INT | Signal::FATAL_ON_QUIT |
  ...
  // Register methods.
  py::class_<VisDQMRenderLink, shared_ptr<VisDQMRenderLink>,
             boost::noncopyable>(
      "DQMRenderLink",
      py::init<std::string, std::string, std::string, int, bool>())
  ...
  py::class_<VisDQMUnknownSource, shared_ptr<VisDQMUnknownSource>,
             py::bases<VisDQMSource>, boost::noncopyable>("DQMUnknownSource",
                                                          py::init<>())
  ...
  py::class_<VisDQMOverlaySource, shared_ptr<VisDQMOverlaySource>,
             py::bases<VisDQMSource>, boost::noncopyable>("DQMOverlaySource",
                                                          py::init<>())
  ...
  py::class_<VisDQMStripChartSource, shared_ptr<VisDQMStripChartSource>,
             py::bases<VisDQMSource>, boost::noncopyable>("DQMStripChartSource",
                                                          py::init<>())
  ...
  py::class_<VisDQMLayoutSource, shared_ptr<VisDQMLayoutSource>,
             py::bases<VisDQMSource>, boost::noncopyable>("DQMLayoutSource")
  ...
  py::class_<VisDQMCertificationWorkspace,
             shared_ptr<VisDQMCertificationWorkspace>,
             py::bases<VisDQMWorkspace>, boost::noncopyable>(
  ...
  py::class_<VisDQMContentWorkspace, shared_ptr<VisDQMContentWorkspace>,
             py::bases<VisDQMWorkspace>, boost::noncopyable>(
      "DQMContentWorkspace",
      py::init<py::object, std::string, std::string, std::string>())
