...
fixStreamerInfo(const Filename &indexdir)
{
  ...
  try
  {
    ...
    DEBUG(1, "adding " << (streamers.size()-1) << " streamers\n");
    writeStrings(wrhead, extended_streamers, VisDQMIndex::MASTER_TSTREAMERINFO);
    ...
    if (f)
    {
           ...
           ! rdhead.isdone(); rdhead.next())
      {
        ...
        if (keyparts[0] == n)
        {
          ...
          me->set_name(objnames.key(keyparts[3]));
          me->set_category(keyparts[1]);
          me->set_lumibegin(keyparts[2]);
