...
void VisDQMLayoutSource::scan(VisDQMItems &result,
  ...
  for (oi = items_.begin(), oe = items_.end(); oi != oe; ++oi) {
    ...
    if (rxsearch && rxsearch->search(o.name.string()) < 0)
      continue;
    if (alarm &&
        ((o.flags & VisDQMIndex::SUMMARY_PROP_REPORT_ALARM) != 0)
        != (*alarm == true))
      continue;
      ...
      for ( ; ri != re; ++ri) {
        ...
        for ( ; ci != ce; ++ci) {
          ...
          if (pycol.ptr() == Py_None)
            ...
            col->path = StringAtom(&stree,
                                   py::extract<std::string>(colobj.get("path")));
